...
import { AddTradePairApi, UpdateTradePairApi } from '/@/api/Manager/CcTradePair';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const getTitle = computed(() => (!unref(isUpdate) ? '新增矿机' : '编辑矿机'));
    ...
    async function handleSubmit() {
      try {
        ...
        // TODO custom api
        if (values.id) {
          await UpdateTradePairApi(values);
        } else {
          await AddTradePairApi(values);
        }
        closeModal();
        emit('success');
      } finally {
